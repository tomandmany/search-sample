...
export default function TableCell({ header, program, participant, participantChannel, setMinHeight, minHeight }: TableCellProps) {
    ...
    const addChannel = async () => {
        ...
        if (response.success) {
            ...
            setChannels([...channels, newChannel]); // IDを設定してからステートを更新
    ...
    const deleteChannel = async (channelId: string) => {
        ...
        if (!response.success) {
            ...
            // 失敗した場合、削除を元に戻す（オプション）
            setChannels(prevChannels => [...prevChannels, channels.find(channel => channel.id === channelId)!]);
        }
    };
                    ...
                    <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
                        {
                            ...
                            <PopoverTrigger asChild>
                                <Button
                                    variant="outline"
                                    className="w-fit mx-auto p-2 flex justify-center items-center gap-3 hover:border-gray-600"
                                >
                                    <PlusIcon />
                                    SNSを追加する
                                </Button>
